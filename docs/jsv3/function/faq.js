...
function faqHTML(data, callback) {
    ...
    $.each(data, function (key, faq) {
        faqhtml += '<div class="panel panel-default ">' +
            '<div class="panel-heading "data-toggle="collapse" data-parent="#faqAccordion" data-target="#question' + removeSpecialChar(faq.id) + '">' +
            '<h4 class="panel-title">' +
            '<a href="#" class="ing">' + faq.question + '</a>' +
            '</h4>' +

            ' </div>' +
            '<div id="question' + removeSpecialChar(faq.id) + '" class="panel-collapse collapse">' +
